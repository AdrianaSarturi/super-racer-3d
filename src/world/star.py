"""
star.py — Estrela 3D Coletável do Super Racer 3D
=================================================
Geometria: cilindro achatado dourado com brilho central.
Animações:
...
from src.world.palette import ESTRELA, ESTRELA_BRILHO, ESTRELA_BORDA
from config import STAR_COLLECT_RADIUS


class Star:
    """
    Estrela 3D coletável com animações de oscilação e brilho pulsante.

    Atributos:
        x, z    -- posição horizontal na arena
        fase    -- offset de fase para variação entre estrelas (evita sincronia)
        ativa   -- False quando coletada (não renderiza nem colide)
    """

    # ── Constantes de animação ─────────────────────────────────────────────────
    ALTURA_BASE    = 0.45   # altura do centro em repouso (acima do chão)
    ...
    # ── Geometria ─────────────────────────────────────────────────────────────
    RAIO          = 0.14    # raio do disco da estrela
    ESPESSURA     = 0.10    # espessura do cilindro
    RAIO_BRILHO   = 0.08    # raio do reflexo central (deve ser < RAIO)
    RAIO_ORBIT    = 0.28    # raio da órbita das partículas
    ...
    def update(self, dt: float, tempo: float, player_pos: rl.Vector3) -> bool:
        ...
        Returns:
            True se a estrela foi coletada neste frame (dist < STAR_COLLECT_RADIUS).
        """
        if not self.ativa:
            return False
    ...
    def draw(self, tempo: float) -> None:
        """Renderiza a estrela animada (chamar dentro de begin_mode_3d)."""
        if not self.ativa:
            return
        ...
        # ── Corpo da estrela (cilindro achatado horizontal) ────────────────
        # draw_cylinder posiciona a base (bottom) em `pos`, cresce para +Y
        pos_base = rl.Vector3(self.x, y - self.ESPESSURA / 2, self.z)
        rl.draw_cylinder(pos_base, self.RAIO, self.RAIO, self.ESPESSURA, 20, ESTRELA)

        # ── Borda escura (cylinder wire levemente maior) ───────────────────
        rl.draw_cylinder_wires(
            pos_base, self.RAIO + 0.01, self.RAIO + 0.01,
            self.ESPESSURA, 20, ESTRELA_BORDA
        )

        # ── Brilho central (disco fino no topo) ───────────────────────────
        brilho_alpha = int(180 + 75 * brilho)
        cor_brilho   = rl.Color(
            ESTRELA_BRILHO.r, ESTRELA_BRILHO.g, ESTRELA_BRILHO.b, brilho_alpha
        )
